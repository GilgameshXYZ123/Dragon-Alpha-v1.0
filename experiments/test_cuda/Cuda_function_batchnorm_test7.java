 ...
 * @author Gilgamesh
 */
public class Cuda_function_batchnorm_test7 
{
...
//        float[] X = Vector.randomFloatVector(length, 0, 1f);
        ...
        Tensor t_Xvar = eg.field_sqmean(tX).c();
        Tensor tY = eg.batchNorm(false, tX, tX_mean, t_Xvar, 1e-5f, tA, tB).c();
        
        //GPU1------------------------------------------------------------------
        Tensor[] grads1 = eg.batchNorm_gradients_v1(false, tdeltaY, tY, t_Xvar, eps, tA, tB);
        Tensor tdeltaX1 = grads1[0].c();
        Tensor tdeltaA1 = grads1[1].c();
        Tensor tdeltaB1 = grads1[2].c();
        
        //GPU2------------------------------------------------------------------
        Tensor[] grads2 = eg.batchNorm_gradients_v2(false, tdeltaY, tX, tX_mean, t_Xvar, eps, tA);
