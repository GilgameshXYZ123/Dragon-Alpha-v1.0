...
public class VGG19 extends Module
{
    ...
    Unit bn5 = nn.batchNorm(false, 512);
    ...
    public Tensor[] __forward__(Tensor... X) {
        ...
        X = F.leakyRelu(conv9.forward(X));//div16, channel: 256 -> 512
        X = F.leakyRelu(conv10.forward(X));
        X = F.leakyRelu(conv11.forward(X));
        X = F.leakyRelu(conv12.forward(X));
        X = bn4.forward(X);
        X = F.maxPool2D(2, X);//div16
      
        X = F.leakyRelu(conv13.forward(X));//div32, channel 512 -> 512
        X = F.leakyRelu(conv14.forward(X));
        X = F.leakyRelu(conv15.forward(X));
        X = F.leakyRelu(conv16.forward(X));
        X = bn5.forward(X);
        X = F.maxPool2D(2, X);
    ...
    String weight = "C:\\Users\\Gilgamesh\\Desktop\\VGG19-cifar10.zip";
