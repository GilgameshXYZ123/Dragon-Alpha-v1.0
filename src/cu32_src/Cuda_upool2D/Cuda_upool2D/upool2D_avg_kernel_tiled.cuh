...
#ifndef UNPOOL_2D_AVERAGE_KERNEL_TILED_H
#define UNPOOL_2D_AVERAGE_KERNEL_TILED_H
...
#ifndef UNPOOL_2D_AVERAGE_KERNEL_TILED_CALL
#define UNPOOL_2D_AVERAGE_KERNEL_TILED_CALL

#define kavg_tiled4(stream, LBY, LBX, ic_index, j_index, deltaY, OH, OW, FH, FW, deltaX, IH, IW, IC, sh, sw, ph, pw, GN, GM)\
	avg_kernel_tiled4\
		<<< dim3(GM>>LBX , GN>>LBY>>2), dim3(1<<LBX, 1<<LBY), 0, stream>>>\
			(deltaY, OH, OW, FH, FW, deltaX, IH, IW, IC, sh, sw, ph, pw, ic_index, j_index)

#define kavg_tiled2(stream, LBY, LBX, ic_index, j_index, deltaY, OH, OW, FH, FW, deltaX, IH, IW, IC, sh, sw, ph, pw, GN, GM)\
	avg_kernel_tiled2\
		<<< dim3(GM>>LBX , GN>>LBY>>1), dim3(1<<LBX, 1<<LBY), 0, stream>>>\
			(deltaY, OH, OW, FH, FW, deltaX, IH, IW, IC, sh, sw, ph, pw, ic_index, j_index)

#define kavg_tiled1(stream, LBY, LBX, ic_index, j_index, deltaY, OH, OW, FH, FW, deltaX, IH, IW, IC, sh, sw, ph, pw, GN, GM)\
	avg_kernel_tiled1\
...
//(Y: BLOCK_SIZE * 4)
#ifndef UNPOOL2D_AVG_KERNEL_TIELD_4
#define UNPOOL2D_AVG_KERNEL_TIELD_4
...
//FH = sh, FW = sw
__global__ void avg_kernel_tiled4(
	...
	int ic_index, int j_index)
{
	...
	float4 dx; float alpha = 1.0f / (FH * FW);
	dx.x = dy.x * alpha;
	dx.y = dy.y * alpha;
	dx.z = dy.z * alpha;
	dx.w = dy.w * alpha;
...
//(Y: BLOCK_SIZE * 2)
#ifndef UNPOOL2D_AVG_KERNEL_TIELD_2
#define UNPOOL2D_AVG_KERNEL_TIELD_2
...
//FH = sh, FW = sw
__global__ void avg_kernel_tiled2(
	const float* __restrict__ deltaY, int OH, int OW,
	int FH, int FW,
	float* __restrict__ deltaX, int IH, int IW,
	int IC,
	...
	int ic_index, int j_index)
{
	...
	float2 dx; float alpha = 1.0f / (FH * FW);
	dx.x = dy.x * alpha;
	dx.y = dy.y * alpha;
...
//(Y: BLOCK_SIZE * 1)
#ifndef UNPOOL2D_AVG_KERNEL_TIELD_1
#define UNPOOL2D_AVG_KERNEL_TIELD_1
...
//FH = sh, FW = sw
__global__ void avg_kernel_tiled1(
	const float* __restrict__ deltaY, int OH, int OW,
	int FH, int FW,
	float* __restrict__ deltaX, int IH, int IW,
	int IC,
	...
	int ic_index, int j_index)
{
	...
	float dx; float alpha = 1.0f / (FH * FW);
	dx = dy * alpha;
