...
#ifndef FIELD_SQUARE_BATCH_NORM_DELTA_AB_V2_H
#define FIELD_SQUARE_BATCH_NORM_DELTA_AB_V2_H
...
#ifndef FIELD_SQUARE_BATCH_NORM_DELTA_AB_V2_CALL
#define FIELD_SQUARE_BATCH_NORM_DELTA_AB_V2_CALL

//LTX >=2
#define field_sqBatchNorm_deltaAB_v2_k4(stream, LBY, LBX, LTY, LTX, deltaY, X, X_mean, X_sqmean, eps, N, M, deltaA, deltaB, width, stride) \
	field_sqBatchNorm_deltaAB_v2_kernel_4<LBY, LBX>\
		<<< dim3(M>>LBX>>LTX, N>>LBY>>LTY), dim3(1<<LBX, 1<<LBY), 0, stream >>>\
			(deltaY, X, X_mean, X_sqmean, eps, N, M, deltaA, deltaB, width, stride)

//[16, 4]
#define field_sqBatchNorm_deltaAB_v2_k4_small(stream, deltaY, X, X_mean, X_sqmean, eps, N, M, deltaA, deltaB, width, stride) \
	field_sqBatchNorm_deltaAB_v2_kernel_4<3, 3>\
...
#ifndef FIELD_SQUARE_BATCH_NORM_DELTA_AB_V2_KERNEL_4
#define FIELD_SQUARE_BATCH_NORM_DELTA_AB_V2_KERNEL_4
...
__global__ void field_sqBatchNorm_deltaAB_v2_kernel_4(
	...
	int width, int stride) 
{
	...
	for (int x4 = offsetX << 2; x4 < M4; x4 += stepX4)
	{
		...
		const float4 x_sqmean = *(float4*)(X_sqmean + x4);
		...
		for (int y = offsetY; y < N; y += stepY)
		{
			...
			Kahan_simdAdd4(v0,   dy, c0);//<4> (deltaXp1 = deltaB) = field_sum: deltaY
...
#ifndef FIELD_SQUARE_BATCH_NORM_DELTA_AB_V2_STAGE
#define FIELD_SQUARE_BATCH_NORM_DELTA_AB_V2_STAGE

void __field_sqBatchNorm_deltaAB_v2_stage(cudaStream_t stream,
	const float* deltaY,
...
{
	if (M > 15) {
		if (N >  63) { field_sqBatchNorm_deltaAB_v2_k4(stream, 3, 2, 3, 2, deltaY, X, X_mean, X_sqmean, eps, N, M, deltaA, deltaB, width, stride); return; }//[64, 16]
		if (N >  31) { field_sqBatchNorm_deltaAB_v2_k4(stream, 3, 2, 2, 2, deltaY, X, X_mean, X_sqmean, eps, N, M, deltaA, deltaB, width, stride); return; }//[32, 16]
		if (N >  15) { field_sqBatchNorm_deltaAB_v2_k4(stream, 3, 2, 1, 2, deltaY, X, X_mean, X_sqmean, eps, N, M, deltaA, deltaB, width, stride); return; }//[16, 16]
	}
	if (M > 7) {
		if (N > 127) { field_sqBatchNorm_deltaAB_v2_k4(stream, 4, 1, 3, 2, deltaY, X, X_mean, X_sqmean, eps, N, M, deltaA, deltaB, width, stride); return; }//[ 64, 8]
		if (N >  63) { field_sqBatchNorm_deltaAB_v2_k4(stream, 4, 1, 2, 2, deltaY, X, X_mean, X_sqmean, eps, N, M, deltaA, deltaB, width, stride); return; }//[ 32, 8]
		if (N >  31) { field_sqBatchNorm_deltaAB_v2_k4(stream, 4, 1, 1, 2, deltaY, X, X_mean, X_sqmean, eps, N, M, deltaA, deltaB, width, stride); return; }//[ 16, 8]
	}
	field_sqBatchNorm_deltaAB_v2_k4_small(stream, deltaY, X, X_mean, X_sqmean, eps, N, M, deltaA, deltaB, width, stride);
}

#endif 


#ifndef FIELD_SQUARE_BATCH_NORM_DELTA_AB_V2
#define FIELD_SQUARE_BATCH_NORM_DELTA_AB_V2

int __field_sqBatchNorm_deltaAB_v2(JNIEnv *env, 
	...
	int partNum)
{
	...
	if (nextN <= partNum) {//V.address = NULL, only 1 stage, write result to Y
		__field_sqBatchNorm_deltaAB_v2_stage(stream1,
