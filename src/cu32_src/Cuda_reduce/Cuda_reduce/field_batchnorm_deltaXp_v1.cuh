...
#ifndef FIELD_BATCH_NORM_DELTAXP_V1_H
#define FIELD_BATCH_NORM_DELTAXP_V1_H
...
#ifndef FIELD_BATCH_NORM_DELTAXP_V1_CALL
#define FIELD_BATCH_NORM_DELTAXP_V1_CALL

//LTX >=2
#define field_batchnorm_deltaXp_v1_k4(stream1, LBY, LBX, LTY, LTX, deltaY, Y, X_mean, X_square_mean, eps, N, M, deltaXp1, deltaXp2, width, stride) \
	field_batchnorm_deltaXp_v1_kernel_4<LBY, LBX>\
		<<< dim3(M>>LBX>>LTX, N>>LBY>>LTY), dim3(1<<LBX, 1<<LBY), 0, stream1 >>>\
			(deltaY, Y, X_mean, X_square_mean, eps, N, M, deltaXp1, deltaXp2, width, stride)

//[16, 4]
#define field_batchnorm_deltaXp_v1_k4_small(stream1, deltaY, Y, X_mean, X_square_mean, eps, N, M, deltaXp1, deltaXp2, width, stride) \
	field_batchnorm_deltaXp_v1_kernel_4<3, 3>\
...
#ifndef FIELD_BATCH_NORM_DELTAXP_V1_KERNEL_4
#define FIELD_BATCH_NORM_DELTAXP_V1_KERNEL_4
...
//[4] X_std[M] = sqrt(X_squareMean - X_mean^2 + eps)
//[5] deltaXp1[M] = field_sum: deltaY * (Y*X_mean - X_std) 
//    deltaXp2[M] = field_sum: deltaY * Y

template<int LBY, int LBX>
__global__ void field_batchnorm_deltaXp_v1_kernel_4(
	...
	int width, int stride)
{
	...
	for (; x4 < M4; x4 += stepX4)
	{
		...
		for (int y = offsetY; y < N; y += stepY)
		{
			...
			float4 dx1;//deltaXp1 = deltaY * (Y*X_mean - X_std) 
			dx1.x = dy.x * (yv.x * x_mean.x - x_std.x);
			dx1.y = dy.y * (yv.y * x_mean.y - x_std.y);
			dx1.z = dy.z * (yv.z * x_mean.z - x_std.z);
			dx1.w = dy.w * (yv.w * x_mean.w - x_std.w);
...
#ifndef FIELD_BATCH_NORM_DELTAXP_V1_STAGE
#define FIELD_BATCH_NORM_DELTAXP_V1_STAGE

//M % 4 == 0, M >= 4
void __field_batchnorm_deltaXp_v1_stage(cudaStream_t stream,
...
{
	if (M > 15) {
		if (N > 63) { field_batchnorm_deltaXp_v1_k4(stream, 3, 2, 3, 2, deltaY, Y, X_mean, X_square_mean, eps, N, M, deltaXp1, deltaXp2, width, stride); return; }//[64, 16]
		if (N > 31) { field_batchnorm_deltaXp_v1_k4(stream, 3, 2, 2, 2, deltaY, Y, X_mean, X_square_mean, eps, N, M, deltaXp1, deltaXp2, width, stride); return; }//[32, 16]
		if (N > 15) { field_batchnorm_deltaXp_v1_k4(stream, 3, 2, 1, 2, deltaY, Y, X_mean, X_square_mean, eps, N, M, deltaXp1, deltaXp2, width, stride); return; }//[16, 16]
	}
	if (M > 7) {
		if (N > 127) { field_batchnorm_deltaXp_v1_k4(stream, 4, 1, 3, 2, deltaY, Y, X_mean, X_square_mean, eps, N, M, deltaXp1, deltaXp2, width, stride); return; }//[ 64, 8]
		if (N >  63) { field_batchnorm_deltaXp_v1_k4(stream, 4, 1, 2, 2, deltaY, Y, X_mean, X_square_mean, eps, N, M, deltaXp1, deltaXp2, width, stride); return; }//[ 32, 8]
		if (N >  31) { field_batchnorm_deltaXp_v1_k4(stream, 4, 1, 1, 2, deltaY, Y, X_mean, X_square_mean, eps, N, M, deltaXp1, deltaXp2, width, stride); return; }//[ 16, 8]
	}
	field_batchnorm_deltaXp_v1_k4_small(stream, deltaY, Y, X_mean, X_square_mean, eps, N, M, deltaXp1, deltaXp2, width, stride);
}

#endif 


#ifndef FIELD_BATCH_NORM_DELTAXP_V1
#define FIELD_BATCH_NORM_DELTAXP_V1

//(correct)
int __field_batchnorm_deltaXp_v1(JNIEnv *env, cudaStream_t stream1, cudaStream_t stream2,
	const float* deltaY, const float* Y,
	const float* X_mean, 
	const float* X_square_mean, float eps,
	int N, int M,
	...
	int partNum)
{
	...
	if (nextN <= partNum) {//V.address = NULL, only 1 stage, write result to Y
		__field_batchnorm_deltaXp_v1_stage(stream1, deltaY, Y, X_mean, X_square_mean, eps, N, M, deltaXp1, deltaXp2, width, stride);
		return nextN;
	}
