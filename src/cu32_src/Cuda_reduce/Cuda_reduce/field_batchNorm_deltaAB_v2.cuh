	...
	int width, int stride)
{
	...
	for (int x4 = offsetX << 2; x4 < M4; x4 += stepX4)
	{
		...
		float4 x_rstd;//<1> X_rstd = rsqrt(X_var + eps)
		...
		if (ty < 2) {
			...
			dB.x *= wrt0; dB.y *= wrt1;//within_width2(result0, xindex2, stride, width);
			dA.x *= wrt0; dA.y *= wrt1;//within_width2(result1, xindex2, stride, width);
