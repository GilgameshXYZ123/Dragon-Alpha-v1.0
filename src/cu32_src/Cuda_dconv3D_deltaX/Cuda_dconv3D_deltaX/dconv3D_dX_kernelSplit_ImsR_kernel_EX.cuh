...
#ifndef DCONV3D_DX_KERNEL_SPLIT_IMSR_KERNEL_EX_H
#define DCONV3D_DX_KERNEL_SPLIT_IMSR_KERNEL_EX_H

//IMS: INPUT_MOD_STEP
//R: remode: W[OC, FH, FW, IC] -> CW[sh, sw, CFH, CFW, OC, IC]
//(IH, IW) % (sh, sw) == 0
//CWstride = CFH * CFW * OC * IC
#ifndef DCONV3D_DX_KERNEL_SPLIT_IMSR_EX_CALL
#define DCONV3D_DX_KERNEL_SPLIT_IMSR_EX_CALL

#define ksIms_88R8_oc2pow(stream, LB, ic_index, j_index, deltaY, OH, OW, CW, FH, FW, deltaX, IH_slice, IW_slice, IC, LOC, sh, sw, ph, pw, GN, GM) \
	ksIms_kernel_8_8R8_OC2pow<LB, (1<<LB>>1)>\
...
#define ksIms_88R4_oc2pow(stream, LB, ic_index, j_index, deltaY, OH, OW, CW, FH, FW, deltaX, IH_slice, IW_slice, IC, LOC, sh, sw, ph, pw, GN, GM) \
	ksIms_kernel_8_8R4_OC2pow<LB, (1<<LB>>1)>\
		<<< dim3((GN>>LB>>3), (GM>>LB>>3), (sh*sw)), dim3(1<<LB, 1<<LB), 0, stream >>>\
			(deltaY, OH, OW, CW, FH, FW, CWstride, deltaX, (IH_slice * IW_slice), IW_slice, IC, LOC, sh, sw, ph, pw, ic_index, j_index)

#define ksIms_88R_oc2pow(stream, LB, ic_index, j_index, deltaY, OH, OW, CW, FH, FW, deltaX, IH_slice, IW_slice, IC, LOC, sh, sw, ph, pw, GN, GM) \
	ksIms_kernel_8_8R_OC2pow<LB, (1<<LB>>1)>\
		<<< dim3((GN>>LB>>3), (GM>>LB>>3), (sh*sw)), dim3(1<<LB, 1<<LB), 0, stream >>>\
			(deltaY, OH, OW, CW, FH, FW, CWstride, deltaX, (IH_slice * IW_slice), IW_slice, IC, LOC, sh, sw, ph, pw, ic_index, j_index)

//========================================================================================
#define ksIms_84R_oc2pow(stream, LB, ic_index, j_index, deltaY, OH, OW, CW, FH, FW, deltaX, IH_slice, IW_slice, IC, LOC, sh, sw, ph, pw, GN, GM) \
	ksIms_kernel_8_4R_OC2pow<LB, (1<<LB>>1)>\
		<<< dim3((GN>>LB>>3), (GM>>LB>>2), (sh*sw)), dim3(1<<LB, 1<<LB), 0, stream >>>\
			(deltaY, OH, OW, CW, FH, FW, CWstride, deltaX, (IH_slice * IW_slice), IW_slice, IC, LOC, sh, sw, ph, pw, ic_index, j_index)

#define ksIms_48R_oc2pow(stream, LB, ic_index, j_index, deltaY, OH, OW, CW, FH, FW, deltaX, IH_slice, IW_slice, IC, LOC, sh, sw, ph, pw, GN, GM) \
	ksIms_kernel_4_8R_OC2pow<LB, (1<<LB>>1)>\
		<<< dim3((GN>>LB>>2), (GM>>LB>>3), (sh*sw)), dim3(1<<LB, 1<<LB), 0, stream >>>\
			(deltaY, OH, OW, CW, FH, FW, CWstride, deltaX, (IH_slice * IW_slice), IW_slice, IC, LOC, sh, sw, ph, pw, ic_index, j_index)

#define ksIms_44R_oc2pow(stream, LB, ic_index, j_index, deltaY, OH, OW, CW, FH, FW, deltaX, IH_slice, IW_slice, IC, LOC, sh, sw, ph, pw, GN, GM) \
	ksIms_kernel_4_4R_OC2pow<LB, (1<<LB>>1)>\
...
#ifndef DCONV3D_DX_KERNEL_SPLIT_IMS_KERNEL_8_8R8_OC_2POW
#define DCONV3D_DX_KERNEL_SPLIT_IMS_KERNEL_8_8R8_OC_2POW
...
__global__ void ksIms_kernel_8_8R8_OC2pow(
	...
	int ic_index, int j_index)
{
	...
	Ys[buf][tx][ty] = Ims4x_loadYs4_oc2pow(deltaY, Y_k, OH, OW, LOC, CFW_OC,
...
//(Y: BLOCK_SIZE*8, X:BLOCK_SIZE*8), OC % (BLOCK_SIZE/2) == 0, OC is power of 2
#ifndef DCONV3D_DX_KERNEL_SPLIT_IMS_KERNEL_8_8R4_OC_2POW
#define DCONV3D_DX_KERNEL_SPLIT_IMS_KERNEL_8_8R4_OC_2POW

//LB = 4: Size = 1.53125, Time = 0.682 msec, Performace = 4821.6 GFlop/s
//LB = 4: Size = 1.125, Time = 0.618 msec, Performace = 3909.25 GFlop/s
//LB = 3: Size = 1.125, Time = 0.66  msec, Performace = 3660.48 GFlop/s
template<int LB, int STEP>
__global__ void ksIms_kernel_8_8R4_OC2pow(
	const float* __restrict__ deltaY, int OH, int OW,
	const float* __restrict__ CW, int FH, int FW, int CWstride,
	float* __restrict__ deltaX, int IH_IW_slice, int IW_slice,
	int IC, int LOC,
...
//(Y: BLOCK_SIZE*8, X:BLOCK_SIZE*8), OC % (BLOCK_SIZE/2) == 0, OC is power of 2
#ifndef DCONV3D_DX_KERNEL_SPLIT_IMS_KERNEL_8_8R_OC_2POW
#define DCONV3D_DX_KERNEL_SPLIT_IMS_KERNEL_8_8R_OC_2POW

//LB = 4: Size = 1.53125, Time = 0.706 msec, Performace = 4657.7 GFlop/s
//LB = 4: Size = 1.125, Time = 0.646 msec, Performace = 3739.81 GFlop/s
//LB = 3: Size = 1.125, Time = 0.688 msec, Performace = 3511.51 GFlop/s
template<int LB, int STEP>
__global__ void ksIms_kernel_8_8R_OC2pow(
	const float* __restrict__ deltaY, int OH, int OW,
	const float* __restrict__ CW, int FH, int FW, int CWstride,
	float* __restrict__ deltaX, int IH_IW_slice, int IW_slice,
	int IC, int LOC,
...
#ifndef DCONV3D_DX_KERNEL_SPLIT_IMS_KERNEL_8_4R_OC_2POW
#define DCONV3D_DX_KERNEL_SPLIT_IMS_KERNEL_8_4R_OC_2POW

//LB = 4: Size = 1.53125, Time = 0.784 msec, Performace = 4194.3 GFlop/s
//LB = 4: Size = 1.125, Time = 0.684 msec, Performace = 3532.05 GFlop/s
//LB = 3: Size = 1.125, Time = 0.784 msec, Performace = 3081.53 GFlop/s
template<int LB, int STEP>
__global__ void ksIms_kernel_8_4R_OC2pow(
	const float* __restrict__ deltaY, int OH, int OW,
	const float* __restrict__ CW, int FH, int FW, int CWstride,
	float* __restrict__ deltaX, int IH_IW_slice, int IW_slice,
	int IC, int LOC,
...
#ifndef DCONV3D_DX_KERNEL_SPLIT_IMS_KERNEL_4_8R_OC_2POW
#define DCONV3D_DX_KERNEL_SPLIT_IMS_KERNEL_4_8R_OC_2POW

//LB = 4: Size = 1.53125, Time = 0.824 msec, Performace = 3990.7 GFlop/s
//LB = 4: Size = 1.125, Time = 0.726 msec, Performace = 3327.71 GFlop/s
//LB = 3: Size = 1.125, Time = 0.792 msec, Performace = 3050.4 GFlop/s
template<int LB, int STEP>
__global__ void ksIms_kernel_4_8R_OC2pow(
	const float* __restrict__ deltaY, int OH, int OW,
	const float* __restrict__ CW, int FH, int FW, int CWstride,
	float* __restrict__ deltaX, int IH_IW_slice, int IW_slice,
	int IC, int LOC,
...
#ifndef DCONV3D_DX_KERNEL_SPLIT_IMS_KERNEL_4_4R_OC_2POW
#define DCONV3D_DX_KERNEL_SPLIT_IMS_KERNEL_4_4R_OC_2POW

//LB = 4: Size = 1.53125, Time = 0.986 msec, Performace = 3335.02 GFlop/s
//LB = 4: Size = 1.125, Time = 0.826 msec, Performace = 2924.84 GFlop/s
//LB = 3: Size = 1.125, Time = 0.954 msec, Performace = 2532.41 GFlop/s
template<int LB, int STEP>
__global__ void ksIms_kernel_4_4R_OC2pow(
	const float* __restrict__ deltaY, int OH, int OW,
	const float* __restrict__ CW, int FH, int FW, int CWstride,
	float* __restrict__ deltaX, int IH_IW_slice, int IW_slice,
	int IC, int LOC,
