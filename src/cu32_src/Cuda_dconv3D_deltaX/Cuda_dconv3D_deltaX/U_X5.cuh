...
__device__ __forceinline__ float4 XKS_SaveYs4(const float* __restrict__ deltaY,
	...
	int yoffset, int Y0, int Y1, int Y2, int Y3)
{
	...
	deltaY = IF_int(ly0, deltaY, ZEROS);
	Y0 = IF_int(ly0, (Y0 + yoffset), (Y0 & 255));
	Y1 = IF_int(ly0, (Y1 + yoffset), (Y1 & 255));
	Y2 = IF_int(ly0, (Y2 + yoffset), (Y2 & 255));
	Y3 = IF_int(ly0, (Y3 + yoffset), (Y3 & 255));
	...
	int ic_index, int j_index)
{
	...
	Ys[buf][tx][ty] = XKS_SaveYs4(deltaY, tohs0, tows0, Y_fhr, Y_fwr, OH, OW,
		yoffset, Y0, Y1, Y2, Y3);
