 ...
 * @author Gilgamesh
 */
public class View extends SimpleInplaceFunction<View>
{
    private int[] inDim;
    private int[] outDim;
    ...
    public final View output_dim(int...outDim) { //outDim = null: the same as flaten
    ...
    protected Tensor __forward__(Tensor X, boolean inplace) {
        ...
        int[] odim = (outDim == null?
                new int[]{ X.dim(0), X.length() / X.dim(0) }:
                Vector.arrayCopy(outDim));
          
         return eg.view(inplace, X, odim);
    }
